...
int main(int argc, char** argv)
{
    try {
        ...
        if (!secret_pattern.empty())
            cout << "Import secret key returned: " << boolalpha << c.import_private_key(secret_pattern) << endl;

        if (!public_pattern.empty())
            cout << "Import public key returned: " << boolalpha << c.import_public_key(public_pattern) << endl;
