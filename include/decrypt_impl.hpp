...
template <typename T> void crypto::decrypt(T passphrase_callback, const data_buffer& in, data_buffer& out)
{
    ...
    if (protocol_ == GPGME_PROTOCOL_OpenPGP)
        gpgme_set_passphrase_cb(ctx.ctx(), callback<T>::passphrase_cb, &passphrase_callback);
    else
