...
public:
    ...
    crypto(gpgme_protocol_t protocol = GPGME_PROTOCOL_OpenPGP, const std::string& keys_directory = "");

public:
    void encrypt(const std::set<std::string>& email_addresses, std::set<std::string>& expired_keys,
                 const data_buffer& in, data_buffer& out);

    template <typename T> void decrypt(T passphrase_callback, const data_buffer& in, data_buffer& out);

    void sign(const std::set<std::string>& email_addresses, const data_buffer& in, data_buffer& out);

    bool verify(const data_buffer& signature, const data_buffer& text);
...
private:
    ...
    bool is_valid_key(gpgme_key_t key, bool secret, bool for_encryption) const;
