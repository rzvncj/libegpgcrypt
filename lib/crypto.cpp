...
crypto::crypto(gpgme_protocol_t protocol,
...
void crypto::encrypt(const std::set<std::string>& email_addresses,
                     ...
                     const data_buffer& in, data_buffer& out)
{
    ...
    if(enc_keys.empty())
        throw runtime_error("key list error: no keys matched the"
                            " supplied patterns");

    err = gpgme_op_encrypt(ctx.ctx(), enc_keys.raw_keys(),
                           GPGME_ENCRYPT_ALWAYS_TRUST,
                           in.data(), out.data());

    if(err != GPG_ERR_NO_ERROR)
        throw runtime_error(string("encrypt error: ") +
                            egpg_gpgme_strerror(err));

    gpgme_encrypt_result_t result = gpgme_op_encrypt_result(ctx.ctx());

    if(result->invalid_recipients)
        throw runtime_error(string("invalid recipients: ") +
                            result->invalid_recipients->fpr);
}


void crypto::sign(const std::set<std::string>& email_addresses,
                  const data_buffer& in, data_buffer& out)
{
    using namespace std;
    ...
    err = gpgme_op_sign(ctx.ctx(), in.data(), out.data(),
                        GPGME_SIG_MODE_DETACH);

    if(err != GPG_ERR_NO_ERROR)
        throw runtime_error(string("sign error: ") +
                            egpg_gpgme_strerror(err));

    gpgme_sign_result_t result = gpgme_op_sign_result(ctx.ctx());

    if(result->invalid_signers)
        throw runtime_error(string("invalid signers: ") +
                            result->invalid_signers->fpr);
}


bool crypto::verify(const data_buffer& signature,
...
{
    bool ret = (!key->revoked && !key->expired && !key->invalid
        && !key->disabled);
...
bool crypto::import_key(const std::string& email, bool secret)
{
    ...
    if(err != GPG_ERR_NO_ERROR)
        throw runtime_error(string("set key list mode error: ") +
                            egpg_gpgme_strerror(err));
    ...
    if(err != GPG_ERR_NO_ERROR)
        throw runtime_error(string("key import error: ") +
                            egpg_gpgme_strerror(err));
